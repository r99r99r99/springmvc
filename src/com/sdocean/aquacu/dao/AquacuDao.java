package com.sdocean.aquacu.dao;
...
import com.sdocean.aquacu.model.AquacuDetailModel;
...
@Component
public class AquacuDao  extends OracleEngine{
	
	private int AquacuDomainId = 12;  //获得海水浴场的功能区ID
	...
	public HchartsPie getAquacuDetailByStation(StationModel station) {
		...
		levelSql.append(" where domainid =  ").append(AquacuDomainId);
		levelSql.append(" order by ordercode");
		levels = this.queryObjectList(levelSql.toString(), Hcharts.class);
		
		AquacuDetailModel bdm = new AquacuDetailModel();
		//查询海水浴场参与评价标准的参数
		...
		indicaSql.append(" and a.domainid = ").append(AquacuDomainId);
		...
		for(IndicatorModel indicator:indicators) {
			//初始化当前参数的游泳指数
			AquacuDetailModel indicatorDetail = new AquacuDetailModel();
			//获得海水浴场功能区中,该参数的配置信息
			List<AquacuDetailModel> aquacuDetails = new ArrayList<AquacuDetailModel>();
			StringBuffer aquacuSql = new StringBuffer("");
			aquacuSql.append(" select distinct a.code as levelid,a.name as levelname,");
			aquacuSql.append(" b.mincal,b.min,b.maxcal,b.max,indicatorcode");
			aquacuSql.append(" from sys_domain_level a,sys_domain_threshold b");
			aquacuSql.append(" where a.id = b.levelid");
			aquacuSql.append(" and a.domainid =").append(AquacuDomainId);
			aquacuSql.append(" and indicatorcode = '").append(indicator.getCode()).append("'");
			aquacuSql.append(" order by a.ordercode ");
			aquacuDetails = this.queryObjectList(aquacuSql.toString(), AquacuDetailModel.class);
			if(aquacuDetails==null||aquacuDetails.size()<1) {
				continue;
			}
			...
			if(dataModel!=null&&dataModel.getIndicator_code()!=null&&dataModel.getIndicator_code().length()>0&&dataModel.getData()!=null) {
				indicatorDetail = this.getAquacuDetailByLevel(aquacuDetails, dataModel);
			}
	 ...
	 */
	public AquacuDetailModel getAquacuDetailByLevel(List<AquacuDetailModel> aquacuDetails,MetadataModel dataModel) {
		AquacuDetailModel result = new AquacuDetailModel();
		...
		for(AquacuDetailModel detail:aquacuDetails) {
			String minCal = ">";
			String maxCal = "<";
		...
		//
		result = this.queryObject(checkSql.toString(), AquacuDetailModel.class);
		return result;
	}
	
	/*
	 * 获得当前站点,统计口径,查询时间内的增养殖区等级趋势
	 */
	public List<StatData> getAquacuStatDataSearchList(AquacuStatisModel gmodel,StationModel station,int type){
		...
		standardSql.append(" and a.levelid = c.id and c.domainid =  ").append(AquacuDomainId);
		...
		for(IndicatorModel indicator:indicators){
			for(MetadataTable tableName:tableList){
				...
				standSql.append(" where b.domainid =  ").append(AquacuDomainId).append(" and b.id = c.levelid");
	 ...
	 * 为水质统计提供饼形图以及统计列表数据
	 */
	public List<StatData> getData4pie(AquacuStatisModel model,StationModel station,int type,List<StatData> statDatas){
		...
		sql.append(" where domainid = ").append(AquacuDomainId);
	...
	/*
	 * 得到站点 参数在统计时间内的所有数据内的水质等级
	 */
	public List<AquacuMetaModel> getAquacuMetaList4StationIndicator(StationModel station,IndicatorModel indicator,String beginTime,String endTime){
		List<AquacuMetaModel>  list = new ArrayList<AquacuMetaModel>();
		List<MetadataTable> tables = tableDao.getTables4Meta(station, beginTime, endTime, 1);
		StringBuffer sql = new StringBuffer("");
		for(int i=0;i<tables.size();i++){
			...
			sql.append(" select collect_time as collecttime,a.wpid as stationid,a.indicator_code as indicatorcode,");
			sql.append(" a.data,b.standard_grade as levelid");
			sql.append(" from ").append(table.getTableName()).append(" a,waterqualitystandard b");
			sql.append(" where wpid = ").append(station.getId());
			sql.append(" and indicator_code = '").append(indicator.getCode()).append("'");
			sql.append(" and collect_time between '").append(beginTime).append("' and '").append(endTime).append("'");
			sql.append(" and a.indicator_code = b.item");
			sql.append(" and b.water_type =").append(station.getWaterType());
			sql.append(" and (");
			sql.append(" case when min=0 then a.data>b.min_value else a.data>=b.min_value end");
			sql.append(" and");
			sql.append(" case when max=0 then a.data<b.max_value else a.data<=b.max_value end");
			sql.append(" )");
			sql.append(" order by a.collect_time desc");
		}
		if(tables!=null&&tables.size()>0){
			list = this.queryObjectList(sql.toString(), AquacuMetaModel.class);
		}
		return list;
	}
}
